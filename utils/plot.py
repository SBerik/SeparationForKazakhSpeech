...
def save_graph_tb_log_metrics(train_csv_path, val_csv_pathn, name_ox, name_oy, loc = 'lower right', pth_save = '../pics/metrics_plot.png'):
    """
    Builds a plot of metric dependencies from two CSV files and saves the image.
    Parameters:
    - train_csv_path (str): Path to the first CSV file (Train).
    - val_csv_pathn (str): Path to the second CSV file (Validation).
    ...
    """
    m_train, m_val  = pd.read_csv(train_csv_path), pd.read_csv(val_csv_pathn)
